...
# CTRL-F - Find file in current directory, recursively
# If git repo, search through all files in git
fzf-find_git() {
  if git rev-parse --git-dir > /dev/null 2>&1; then
   # This is a valid git repository (but the current working
    # directory may not be the top level.
  ...
  RESULT=$(builtin cd $WORKING_PATH && echo $(__fsel))

  if [ -z "$RESULT" ]
  then
    LBUFFER=$RESULT
  else
    LBUFFER="code $(realpath --relative-to="${PWD}" "${WORKING_PATH}/$RESULT")"
  fi
  local ret=$?
  zle reset-prompt
  return $ret
...
zle -N fzf-find_git
bindkey '^T' fzf-find_git

# ALT-C - cd into the selected directory
# If git repo, search through all folder in git
fzf-cd-git() {
  if git rev-parse --git-dir > /dev/null 2>&1; then
   # This is a valid git repository (but the current working
    # directory may not be the top level.
  ...
  if [ -z "$BUFFER" ]; then
    BUFFER="cd $(realpath --relative-to="${PWD}" "${WORKING_PATH}/${(q)dir}")"
    zle accept-line
  else
    print -sr "cd ${(q)dir}"
    cd "$dir"
